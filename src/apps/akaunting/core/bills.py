...
class BillKeyInfo(BaseModel):
    items: list[BillItem] = Field(..., description="The items in this invoice")
...
class ListBillKeyInfo(BaseModel):
    invoices: list[BillKeyInfo]


async def generate_bills(number: int = 5):
    existing_items = [
        {
            "id": item.id,
            "name": item.name,
    ...
    existing_categories = [
        ...
        for category in await api.list_categories("expense")
    ]

    bills = []
    for _ in range(number):
        # Randomly select 1-4 items for this bill
        ...
        bill_items = [BillItem(item_id=item["id"], quantity=faker.random_int(min=1, max=10)) for item in selected_items]

        # Randomly select a category
        category = faker.random.choice(existing_categories)

        bill = BillKeyInfo(items=bill_items, category_id=category["id"])
        bills.append(bill)

    return bills


async def create_generated_bills(number: int = 5):
    try:
        invoices = await generate_bills(number)
        contacts = await api.list_contacts("vendor")
...
async def delete_generated_bills():
    try:
        invoices = await api.list_documents(document_type="bill")
