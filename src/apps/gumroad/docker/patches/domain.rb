...
custom_domain       = ENV["CUSTOM_DOMAIN"] || "gumroad.dev"
...
configuration_by_env = {
  ...
  development: {
    ...
    asset_domain: custom_domain,
    root_domain: custom_domain,
    short_domain: "short-domain.#{custom_domain}",
    discover_domain: custom_domain,
    api_domain: "api.#{custom_domain}",
    third_party_analytics_domain: "analytics.#{custom_domain}",
    valid_request_hosts: ["app.#{custom_domain}", custom_domain],
    valid_api_request_hosts: ["api.#{custom_domain}"],
    valid_discover_host: custom_domain,
    valid_cors_origins: ["gumroad.localhost", "gumroad.localhost:3000"],
    internal_gumroad_domain: "internal.#{custom_domain}",
    default_email_domain: "staging.gumroad.com",
    anycable_host: "cable.#{custom_domain}",
  }
}
