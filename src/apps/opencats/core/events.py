...
async def seed_events() -> dict[str, Any]:
            ...
            try:
                ...
                if result and result.get("status_code") == 200:
                    ...
                    if entity_id:
                        ...
                        seeded_events.append({"original_data": event, "opencats_id": entity_id, "status": "success"})
                        seeded_count += 1
                    else:
                        logger.warning(f"⚠️ Event '{event_title}' may have been created but ID not found")
                        seeded_events.append({"original_data": event, "opencats_id": None, "status": "unknown"})
                        seeded_count += 1
                else:
                    logger.error(f"❌ Failed to seed event '{event_title}': {result}")
                    seeded_events.append({"original_data": event, "opencats_id": None, "status": "failed", "error": str(result)})
                    error_count += 1

            except Exception as e:
                logger.error(f"❌ Error seeding event '{event_title}': {e!s}")
                seeded_events.append({"original_data": event, "opencats_id": None, "status": "error", "error": str(e)})
                error_count += 1
    ...
    return {"seeded_events": seeded_count, "errors": error_count, "details": seeded_events}
