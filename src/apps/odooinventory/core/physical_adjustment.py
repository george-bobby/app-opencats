    ...
    async with OdooClient() as client:
        for prod, location in items_chunk:
            try:
                ...
                logger.error(f"Error processing adjustment for product {prod.get('name')} at location {location.get('name')}: {e}")
...
async def insert_physical_adjustments():
    ...
    await process_in_parallel(items=items_to_process, async_processor=_process_adjustment_chunk)
