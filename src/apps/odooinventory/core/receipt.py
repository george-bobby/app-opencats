...
async def _process_receipt_chunk(
    ...
    async with OdooClient() as client:
        for picking_type, product, receipt_picking_type_id, partners in items_chunk:
            try:
...
async def insert_receipts():
    ...
    await process_in_parallel(items=items_to_process, async_processor=_process_receipt_chunk)

    logger.succeed(f"Finished inserting receipts for warehouses: {', '.join([w['code'] for w in warehouses if w.get('code')])}")


async def _process_diversify_chunk(pickings_chunk: list[dict[str, Any]]):
    """Processes a chunk of pickings to diversify their statuses."""
    async with OdooClient() as client:
        for picking in pickings_chunk:
            try:
...
async def diversify_receipt_statuses():
    ...
    pickings = pickings[: int(len(pickings) * 0.4)]  # Process only 50% of the receipts

    await process_in_parallel(items=pickings, async_processor=_process_diversify_chunk)
