"""Manages partner industry data for company classification."""

from apps.odoohr.config.constants import ResModelName
...
async def get_industries():
    async with OdooClient() as client:
        industries = await client.search_read(ResModelName.PARTNER_INDUSTRY.value, [], ["id", "name"])
        return industries
...
async def insert_industry():
    ...
    async with OdooClient() as client:
        ...
        if settings.DATA_THEME_SUBJECT in [i["name"] for i in industries]:
            return
        await client.create(ResModelName.PARTNER_INDUSTRY.value, {"name": settings.DATA_THEME_SUBJECT})
