...
class AsyncPostgresClient:
    ...
    @classmethod
    async def execute_bulk(cls, query: str, *args) -> str:
        """Execute a bulk query with optimized connection settings."""
        pool = await cls.get_pool()
    ...
    async def get_non_admin_users(cls) -> list[dict[str, Any]]:
        """Get all users where email is not admin@acme.inc"""
        query = "SELECT * FROM users WHERE email != $1"
        return await cls.fetch(query, "admin@acme.inc")
