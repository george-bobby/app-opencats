...
def create_client(
    ...
    def impersonate(email: str):
        return client.session.post(
            url=f"{settings.API_URL}/api/method/frappe.core.doctype.user.user.impersonate",
            data={"user": email, "reason": "Seeding data"},
        )

    client.insert_many = insert_many_json
    client.assign = assign
    client.impersonate = impersonate
    return client
